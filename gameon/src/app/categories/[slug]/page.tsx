...
const GameCategory =async (props:{params:{slug: string}}) => {
    ...
    console.log(games)
    // const {subtitle, image} = await getCategroy(slug);
  return (
  <>
            ...
            <div className="lg:w-3/4">
                ...
                <p className={classNames.subtitle}>
                   {(games && games.length>0)?(
                    <div>
                        {games[0].category?.subtitle}
                    </div>
                   ):(<div>
                    A short subtitle that provides more context about the game.
                   </div>)}
                       </p>
            </div>
        </div>
    ...
    <section className={classNames.section}>
        ...
        <div className="flex rounded gap-8 flex-wrap py-10">
            {/* colleciton games */}
            {games.map((games)=>(
                <GameCard key={games._id} gameName={games.name} imageUrl={games.image[0].url} price={games.price} slug={games.slug.current}/>
            ))}
        </div>
